...
\noindent
The basic idea behind embedding a file system within a video is to break files written to the volume into many \textit{chunks} and embed these into the video frames. Some kind of header will need to be developed to keep track of the locations of the chunks so that each file can be rad back correctly.
...
\begin{lstlisting}[language=C++, caption={\texttt{FUSE} readdir implementation (\texttt{fs/stegfs.cc:264}).}, frame=single, label=readdir,upquote=true,float,floatplacement=!htb]
...
\noindent
The performance of \texttt{Stegasis} operating on \texttt{JPEG} images is noticeably slower compared to uncompressed \texttt{AVI} files. This is due to the decompressing and recompressing of the \texttt{JPEG} frames. As mentioned in the preparation section, it is not possible to hold all of the decompressed \texttt{JPEG} frames in memory, meaning they will need to remain compressed, only being decompressed when requested.\\
