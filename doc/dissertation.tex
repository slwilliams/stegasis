...
\noindent
The results of this evaluation study obtained by the implemented web application are discussed in Section \ref{security} and conclude that it was not possible to visually differentiate between the cover and stego objects at the default capacity setting of 20\%.
...
The above unit test verifies the steganogrphic correctness of the stego systems, checking that data embedded using a key is correctly extracted when using the same key.
...
With the exception of the Longest Runs test (which I believe has failed due to the small sample size), these results strongly indicate that the bit string is random and therefore that the users could not differentiate between the cover and stego objects produced by \texttt{Stegasis}.
...
610+0 records in
610+0 records out
79953920 bytes (80 MB) copied, 1.037613 s, 77.1 MB/s
\end{lstlisting}
...
Listing \ref{suite} shows the test suite which is executed for each steganographic algorithm and combination of cryptographic algorithm.
