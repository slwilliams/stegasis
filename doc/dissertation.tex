...
\item Allow users to embed data within video files:
	\begin{enumerate}
		...
		\end{center}
		\item Embedded should occur with no perceivable impact on video quality.
...
\item Provide performance adequate for normal use:
	\begin{enumerate}
		...
		\item Typical bitrates of full HD video are 8 - 12 Mb/s and therefore the file system should at a minimum give read speeds of 12 Mb/s.
	\end{enumerate}		
\end{enumerate}
...
Some interesting points to note about decompressing to ram etc...
...
To evaluate the claim ``Embedding 
...
\subsubsection{Adequate performance}

The performance of the file system allows full HD video content to be played back directly from within the volume. The file system provides read (and write) speeds greater that the stated minimum 12 Mb/s, satisfying the final core requirement. Performance is discussed in detail below in section \ref{perf}.\\
...
This extension task was investigated but was decided not to be implemented.  
...
A website was implemented and hosted for the majority of the duration of the project. A total of *2000* data points were collected from 15 individual users. The results are discussed in detail below in section \ref{security}, but the outcome of the study confirms that it is not possible to visually differentiate videos produced by \texttt{Stegasis} when using the default capacity setting (20\%). This satisfies the final extension task.\\

\noindent
\texttt{Stegasis} therefore has also satisfied all\footnote{I'm considering the audio extension task as satisfied since it was investigated and chosen not to be implemented.} of the proposed extension tasks.

\subsection{Security}
\label{security}

Not that secure but can't see, website thing.
