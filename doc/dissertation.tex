...
A \texttt{Chunk} abstracts the concept of a single frames video data. In the case of the uncompressed \texttt{AVI}, this can be thought of as a \texttt{char} pointer to the GBR pixel data, along with an associated frame size in bytes. The commonly used technique of only writing back dirty data is applied here, associating a boolean with each \texttt{Chunk} which is true if and only if the frame data has been modified.  
...
The \texttt{AVI} parssing process can be thought of consisting of two main parts; parsing the video headers and parsing the video chunk data. The following pseudocode illustrates the parsing process with the headers being parsed lines 1-12 and the chunks being parsed lines 15-22: 
...
The effect of the capacity parameter is illustrated within figure \ref{cap50}. The left image is the original LSB plane (red channel) of a frame taken from an uncompressed \texttt{AVI} video. The right image is the same frame with data embedded within it using a capacity setting of 50\%.

\begin{figure}[here]
\centerline{\includegraphics[width=0.9\textwidth]{images/cap50comp.png}}
