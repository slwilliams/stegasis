...
class SteganographicAlgorithm {
  protected:
    ...
    VideoDecoder *dec;
  public:
    virtual int embed(Frame *f,char *data,int reqByteCount,int offset) = 0;
...
\noindent
The \texttt{embed} function should be read as ``attempt to embed \texttt{reqByteCount} bytes from \texttt{data} into frame \texttt{f} starting at an offset \texttt{offset} bytes into the frame'' and returns the number of bytes successfully embedded. Similarly, the \texttt{extract} function should be read as ``attempt to extract \texttt{reqByteCount} bytes from frame \texttt{f} starting at an offset \texttt{offset} bytes into the frame and put them into \texttt{output}'' and returns the number of bytes successfully extracted and the current frame offset.
