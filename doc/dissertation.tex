...
This expression arises from the fact that uncompressed \texttt{AVI} uses 3 bytes per pixel value. Since there are $height \cdot width$ pixels within a single frame, multiplying this by 3 will give the total number of bytes. \textit{Capacity} is a user provided percentage ranging in value from 1 - 100. It specifies the percentage of the frame to embed within. \texttt{frameSize} must therefore reduce the returned frame size value by ``capacity percent''.
...
\begin{lstlisting}[caption={\texttt{FFmpeg} frame extraction and reassembly command.}, frame=single, label=ffmpegcmd,upquote=true,float,floatplacement=!htb]
...
\begin{lstlisting}[caption={\texttt{FUSE} read function callback.}, frame=single, label=readfs,showstringspaces=false,upquote=true]
...
\begin{lstlisting}[caption={\texttt{FUSE} write function callback.}, frame=single, label=writefs,showstringspaces=false,upquote=true]
