...
\textsc{Year}: \tab{2015}\\
\textsc{Word Count}: \tab{13,281 (cut out 477)}\\
...
random excursion (variant) test is approximately = 47 for a
...
The data points for the graph were obtained using the command line tool \texttt{dd} as shown in Listing \ref{dd}.

\begin{lstlisting}[language={},caption={Testing the file system performance using \texttt{dd}.}, frame=single, label=dd,]
$ dd if=/tmp/randomData of=test bs=65k count=1221
610+0 records in
610+0 records out
79953920 bytes (80 MB) copied, 0.918484 s, 87.1 MB/s
\end{lstlisting}
...
The \texttt{LSB} algorithm gives write speeds on par with SATA hard drives\textsuperscript{\cite{hdd}} far exceeding those of USB 2.0 and even 3.0 devices.\textsuperscript{\cite{usb3}} The \texttt{LSBP} algorithm is also almost on par with USB 2.0 devices. However the algorithms involving decompressing \texttt{JPEG} images offer a lot worse file system performance. 
