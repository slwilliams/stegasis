...
I<position>. Note the in most formats it is not possible to seek exactly, so
B<ffmpeg> will seek to the closest seek point before I<position>.
...
I<date> must be a time duration specification,
...
Print sdp information to I<file>.
...
For example to copy with B<ffmpeg> the entire Audio-CD in /dev/sr0,
...
C<avfilter_graph_parse()>/C<avfilter_graph_parse2()> functions defined in
...
Do not use C<fieldmatch> and B<decimate> if your input has mixed
...
=head2 color, haldclutsrc, nullsrc, rgbtestsrc, smptebars, smptehdbars, testsrc
...
section in the ffmpeg-utils manual. Default value is C<400x400>.
...
Specify the red, green and blue contrast. Default values are C<40>, C<160> and C<80>.
...
Specify the red, green and blue fade. Default values are C<15>, C<10> and C<5>.
Allowed range is C<[0, 255]>.
...
and minimum resolution is C<640x480>.
